...
class local_policy_att(nn.Module):
    ...
    def forward(self, dist, theta, ninf_mask=None, xy=None):
        ...
        #cur_token 120 100 32  #init_k 120 100 30 32
        ...
        # shape: (batch, head_num, multi, local, qkv_dim) 120 4 100 30 8
...
class local_policy(nn.Module):
    def __init__(self, model_params, idx=0):
        ...
        self.penalty = model_params['xi']
        self.local_size = model_params['local_size'][idx]
        self.Linear_1 = nn.Linear(2 * self.local_size + 1, emb_dim)
    ...
    def forward(self, dist, theta, scale):
        # theta shape: (batch, multi, problem)
        # dist shape: (batch, multi, problem)
        padding = False
...
class local_policy_AFT(nn.Module):
    def __init__(self, model_params, idx=0):
        super().__init__()
        self.emb_dim = model_params['local_att_hidden_dim']
        self.head_num = model_params['local_att_head_num']
    ...
    def forward(self, dist, theta, ninf_mask=None, xy=None):
        ...
        A_norm_fac = (sorted_dist.max(-1)[0].unsqueeze(-1) + 1e-6)
        A_dist = torch.take_along_dim(xy[:, :, :, 0], idx, dim=-1) / A_norm_fac
        ...
        q = self.Wq(cur_token).unsqueeze(2)
        K = self.Wk(init_k)
        V = self.Wv(init_k)
        Q_sig = torch.sigmoid(q)
        A = (-self.alpha * A_dist).unsqueeze(-1).permute(0, 1, 3, 2)
        temp = torch.exp(A) @ torch.mul(torch.exp(K), V)
        weighted = temp / (torch.exp(A) @ torch.exp(K))
        out = torch.mul(Q_sig, weighted).squeeze(2)
...
def _get_encoding(encoded_nodes, node_index_to_pick):
    # encoded_nodes.shape: (batch, problem, embedding) 120 100 128
    # node_index_to_pick.shape: (batch, pomo) 120 100
    ...
    # shape: (batch, pomo, embedding)  120 100 128
...
class TSP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        #RelD的添加
        # self.Widt = nn.Linear(head_num * qkv_dim, embedding_dim,  bias=False)
        self.ff = Feed_Forward_Module(**model_params)


    def add_local_policy(self, device, idx=0):
        #self.local_policy_0 = local_policy_AFT(self.model_params).to(device)
        self.local_policy_0 = local_policy(self.model_params).to(device)
    ...
    def forward(self, encoded_last_node, cur_dist, cur_theta, xy, ninf_mask,scale=None):
        ...
        q_res = self.Wq_last(encoded_last_node)

        q_last = reshape_by_heads(q_res, head_num=head_num)
        # shape: (batch, head_num, pomo, qkv_dim)  120 8 100 16

        #h_c
        q = self.q_first + q_last
        # shape: (batch, head_num, pomo, qkv_dim)   120 8 100 16

        out_concat = multi_head_attention(q, self.k, self.v, rank3_ninf_mask=ninf_mask) + q_res
        # shape: (batch, pomo, head_num*qkv_dim)  120 100 128

        #h_c^' (query vector)
        mh_atten_out = self.multi_head_combine(out_concat)
        # shape: (batch, pomo, embedding) 120 100 128

        final_mh_atten_out = self.ff(mh_atten_out) + mh_atten_out

        #  Single-Head Attention, for probability calculation
        #######################################################
        score = torch.matmul(final_mh_atten_out, self.single_head_key)
        ...
        if self.model_params['ensemble'] and self.local:
            ...
            score_scaled += self.local_policy_0(dist=cur_dist, theta=cur_theta, scale=scale)
