...
def rollout(model, env, eval_type='greedy'):
    ...
    while not done:
        cur_dist, cur_theta, xy ,scale  = env.get_local_feature()
        selected, one_step_prob = model.one_step_rollout(state, cur_dist=cur_dist, cur_theta=cur_theta, xy=xy, eval_type=eval_type,scale=scale)
