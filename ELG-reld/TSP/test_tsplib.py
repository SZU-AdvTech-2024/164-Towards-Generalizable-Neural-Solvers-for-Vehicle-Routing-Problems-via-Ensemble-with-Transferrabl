...
import torch
import numpy as np
from torch.optim import Adam as Optimizer5000
...
class TSPLib_Tester:
    ...
    def test_on_tsplib(self):
        ...
        print("Total average gap {:.2f}%".format(100 * ((np.array(total_cost) - np.array(opt)) / np.array(opt)).mean()))
        print("<=200 average gap {:.2f}%".format(100 * ((np.array(small_cost) - np.array(small_opt)) / np.array(small_opt)).mean()))
        print("200-500 average gap {:.2f}%".format(100 * ((np.array(medium_cost) - np.array(medium_opt)) / np.array(medium_opt)).mean()))
        print("500-1002 average gap {:.2f}%".format(100 * ((np.array(large_cost) - np.array(large_opt)) / np.array(large_opt)).mean()))
