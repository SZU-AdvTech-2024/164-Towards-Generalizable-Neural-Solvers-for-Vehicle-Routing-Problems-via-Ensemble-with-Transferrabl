...
def validate(model, multiple_width, device, mixed=True):
    ...
    if mixed:
        # validation dataset
        val_u = VRPDataset('data/vrp_uniform100_1000_seed1234.pkl', num_samples=1000)
        val_u_loader = DataLoader(val_u, batch_size=1000)
        val_c = VRPDataset('data/vrp_cluster100_1000_seed1234.pkl', num_samples=1000)
        val_c_loader = DataLoader(val_c, batch_size=1000)
        val_m = VRPDataset('data/vrp_mixed100_1000_seed1234.pkl', num_samples=1000)
...
def train(model, training, T, start_steps, train_steps, mixed, train_batch_size, problem_size, distribution, multiple_width, lr, device, logger, scale_norm, fileLogger, dir_path, log_step):
    # Initialize env
    env = CVRPEnv(multi_width=multiple_width, device=device)
    ...
    for i in trange(train_steps - start_steps + 1):
        ...
        if i == T - start_steps - 1 and training == 'joint':
