...
class CVRP_Decoder(nn.Module):
    ...
    def add_local_policy(self, device):
        ...
        self.local_policies = nn.ModuleList(
            [local_policy(self.model_params, idx=i).to(device) for i in range(self.model_params['ensemble_size'])])
        self.local = True
    ...
    def forward(self, encoded_last_node, load, cur_dist, cur_theta, xy, norm_demand, ninf_mask,ins_feature=None):
        ...
        if self.model_params['ensemble'] and self.local:
            ...
            for i in range(self.model_params['ensemble_size']):
                ...
                score_local += self.local_policies[i](theta=cur_theta, dist=cur_dist, ins_feature=ins_feature)
