...
class CVRPEnv:
    ...
    def get_instance_feature(self):
        # self.depot_node_demand.sum(-1)  batch_size个demand的和   shape: batch_size
        #
        route_length = self.problem_size / self.depot_node_demand.sum(-1)
        route_length = route_length[:, None, None].expand(self.batch_size, self.multi_width, 1)
        # shape: (batch, multi, 1)
        scale = self.problem_size * torch.ones((self.batch_size, self.multi_width, 1), device=self.device)
        # shape: (batch, multi, 1)

        return [route_length, scale]